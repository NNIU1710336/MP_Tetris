...
{
    m_joc.inicialitza(mode, fitxerInicial, fitxerFigures, fitxerMoviments);
    m_joc.dibuixa();
}
...
void Partida::Nivell()
{
    ...
    //Dividir els punts actuals entre 1000 i guardar-ho en una variable, si aquesta variable es superior a l'anterior s'augmenta en un la dificultat
}
...
void Partida::actualitza(double deltaTime)
{
    ...
    Puntuacio();
    Nivell();
    string missatge = "Puntuacio: " + to_string(m_puntuacio);
    string missatge4 = "Nivell: " + to_string(m_nivell);
    GraphicManager::getInstance()->drawFont(FONT_GREEN_30, POS_X_TAULER, POS_Y_TAULER - 50, 0.75, missatge);
    GraphicManager::getInstance()->drawFont(FONT_GREEN_30, POS_X_TAULER, POS_Y_TAULER - 30, 0.75, missatge4);
